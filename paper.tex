...
A: $N_A=10 \Rightarrow \Delta s_A = 10\,\lambda_A$\\
...
and \(c=\lambda_B/\tau_B\). This identifies time a \emph{dynamic axis} increase generated by
...
\(\Delta\tau = \Delta t^{(B)}\sqrt{1-\frac{v^2}{c^2}} = \gamma^{-1}\,\Delta t^{(B)}\), so
\[
